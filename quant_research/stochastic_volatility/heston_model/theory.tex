...
\section{Numerical Validation and Diagnostics}

This section presents comprehensive computational validation of the Heston model's theoretical properties and Monte Carlo implementation. All results were generated using the Euler-Maruyama discretization scheme with careful attention to the Feller condition and variance process stability.

\subsection{Parameter Sensitivity Analysis}

Systematic sensitivity analysis confirms the theoretical roles of each Heston parameter and validates key model predictions:

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{diagnostics_heston_sensitivity.pdf}
\caption{Heston parameter sensitivity analysis across the volatility smile (30,000 Monte Carlo paths per strike). \textbf{Top-left:} Initial variance $V_0$ shifts the smile level. \textbf{Top-right:} Correlation $\rho$ controls skew strength and direction. \textbf{Bottom-left:} Mean reversion $\kappa$ affects smile shape through vol persistence. \textbf{Bottom-right:} Vol-of-vol $\xi$ amplifies smile curvature (note: $\xi = 0.5$ violates Feller condition).}
\label{fig:heston_sensitivity}
\end{figure}

\noindent \textbf{Key findings:}
\begin{itemize}
    \item \textbf{$V_0$ vs $\theta$ redundancy}: Both parameters shift implied volatility levels uniformly. In practice, $V_0$ is typically fixed to match current ATM implied variance, while $\theta$ captures the long-term equilibrium level. They exhibit strong negative correlation during calibration.
    
    \item \textbf{$\rho$ (correlation)}: Negative correlation (typical $\rho \approx -0.7$ for equities) produces the characteristic downward-sloping skew where OTM puts are more expensive than OTM calls. This captures the leverage effect: as prices fall, volatility rises.
    
    \item \textbf{$\kappa$ (mean reversion)}: Controls how quickly volatility returns to $\theta$ after shocks. Higher $\kappa$ produces flatter term structures and less persistent volatility clustering. Values between 1-5 are typical for equity markets.
    
    \item \textbf{$\xi$ (vol-of-vol)}: Controls smile convexity. However, high $\xi$ risks violating the Feller condition ($2\kappa\theta \geq \xi^2$), potentially causing variance to reach zero in extreme scenarios. Our diagnostics flag violations with runtime warnings.
\end{itemize}

\subsection{Feller Condition Monitoring}

The Feller condition ($2\kappa\theta \geq \xi^2$) is critical for ensuring variance remains strictly positive. Our implementation includes automatic checking:

\begin{table}[h]
\centering
\caption{Feller Condition Compliance in Sensitivity Analysis}
\label{tab:feller_check}
\begin{tabular}{lccc}
\hline
\textbf{Parameter Set} & \textbf{$2\kappa\theta$} & \textbf{$\xi^2$} & \textbf{Status} \\\\
\hline
Standard Heston & 0.080 & 0.040 & ✓ Pass \\\\
High vol-of-vol ($\xi=0.5$) & 0.080 & 0.250 & ✗ \textbf{Violation} \\\\
Low mean reversion ($\kappa=0.5$) & 0.020 & 0.040 & ✗ \textbf{Violation} \\\\
Conservative ($\xi=0.1$, $\kappa=3$) & 0.120 & 0.010 & ✓ Pass \\\\
\hline
\end{tabular}
\end{table}

\noindent When calibrating to market data, prioritize parameter sets satisfying the Feller condition. Violations may produce stable simulations for typical paths but can fail catastrophically during stress scenarios or long-dated simulations.

\subsection{Monte Carlo Error Asymmetry}

A critical discovery from our diagnostics is the \textbf{asymmetric error behavior} between calls and puts in Monte Carlo pricing. This has important implications for computational efficiency:

\begin{table}[h]
\centering
\caption{Monte Carlo Error Characteristics by Option Type}
\label{tab:mc_asymmetry}
\begin{tabular}{lcccc}
\hline
\textbf{Volatility Regime} & \textbf{Call RMSE} & \textbf{Put RMSE} & \textbf{Error Ratio} & \textbf{Advantage} \\\\
\hline
Low vol ($\sigma = 15\%$) & 0.00282 & 0.00025 & \textbf{11.3$\times$} & Puts \\\\
Medium vol ($\sigma = 25\%$) & 0.00198 & 0.00053 & 3.7$\times$ & Puts \\\\
High vol ($\sigma = 40\%$) & 0.00315 & 0.00181 & 1.7$\times$ & Puts \\\\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{diagnostics_mc_error_asymmetry.pdf}
\caption{Monte Carlo error asymmetry across volatility regimes (10,000 simulations per test case). \textbf{Top-left:} High volatility amplifies absolute errors. \textbf{Top-right:} Error distributions widen significantly with volatility. \textbf{Bottom-left:} Errors remain approximately unbiased (mean near zero). \textbf{Bottom-right:} Error magnitudes concentrate in predictable ranges by regime.}
\label{fig:mc_asymmetry}
\end{figure}

\noindent \textbf{Explanation of asymmetry:}
\begin{itemize}
    \item \textbf{Put options} (especially OTM) have bounded payoffs $[0, K-S_T]$ with rapid decay as $S_T$ increases. Monte Carlo paths in the upper tail contribute near-zero to the expectation, reducing variance.
    
    \item \textbf{Call options} (especially OTM) have unbounded payoffs $[0, S_T-K]$ that grow linearly with extreme paths. Upper-tail simulation errors have disproportionate impact, increasing variance.
    
    \item \textbf{Practical implication}: When pricing portfolios, use put-call parity to price OTM puts directly and derive OTM calls synthetically. This can reduce required path counts by 50-70\% for equivalent accuracy.
\end{itemize}

\subsection{Calibration Efficiency}

Standard global optimization methods (differential evolution, simulated annealing) are robust but computationally expensive. We tested smart initialization strategies for local optimization:

\begin{table}[h]
\centering
\caption{Heston Calibration Performance}
\label{tab:heston_calibration}
\begin{tabular}{lccc}
\hline
\textbf{Method} & \textbf{Time} & \textbf{Speedup} & \textbf{Fit Quality (RMSE)} \\\\
\hline
Global DE (baseline) & 890 ms & 1.0$\times$ & 0.0031 \\\\
Local + Smart init & \textbf{78 ms} & \textbf{11.4}$\times$ & 0.0034 \\\\
Local + Random init & 72 ms & 12.4$\times$ & 0.0189 \\\\
\hline
\end{tabular}
\end{table}

\noindent \textbf{Smart initialization heuristics:}
\begin{enumerate}
    \item $V_0 \approx \sigma_{ATM}^2$: Match current ATM implied variance
    \item $\theta \approx V_0 \times 1.2$: Assume slight upward term structure
    \item $\kappa \approx 2.0$: Moderate mean reversion (typical equity value)
    \item $\xi \approx 0.3$: Conservative vol-of-vol ensuring Feller compliance
    \item $\rho \approx -0.7$: Standard equity leverage effect
\end{enumerate}

These heuristics capture market intuition and domain knowledge, enabling 11.4$\times$ faster calibration while maintaining 91\% of global optimizer quality.

\subsection{Variance Process Stability}

The CIR variance process requires careful numerical treatment. We validated the full truncation scheme (set $v_t = \max(v_t, 0)$ after each step):

\begin{itemize}
    \item \textbf{Absorption scheme}: Never apply negative variance by truncating at zero. This introduces slight positive bias but ensures numerical stability.
    
    \item \textbf{Reflection scheme}: Reflect negative variance to positive values. This preserves distributional properties better but can create artificial oscillations.
    
    \item \textbf{Recommendation}: Use absorption (truncation) for production systems due to simplicity and robustness. For research requiring exactness, consider the Quadratic Exponential (QE) scheme or exact simulation methods.
\end{itemize}

\subsection{Computational Recommendations}

Based on validation results, we recommend:

\begin{enumerate}
    \item \textbf{Path counts}: Use minimum 10,000 paths for vanilla pricing, 50,000+ for exotics, and 100,000+ for risk sensitivities (Greeks).
    
    \item \textbf{Time discretization}: Use $\Delta t \leq 1/52$ (weekly steps) for standard maturities. Increase to daily steps for short-dated options ($T < 3$ months).
    
    \item \textbf{Variance control}: Always enforce $v_t \geq 0$ via truncation. Monitor Feller condition violations during calibration.
    
    \item \textbf{Calibration}: Use smart initialization + local optimization for daily recalibration (70-100ms per surface). Reserve global methods for monthly deep calibration.
    
    \item \textbf{Error budgeting}: Exploit put-call asymmetry by pricing OTM puts directly and using parity for calls. This provides 3-11$\times$ error reduction in low-volatility regimes.
\end{enumerate}

\subsection{Comparison with SABR}

While both are stochastic volatility models, key differences emerged:

\begin{table}[h]
\centering
\caption{Heston vs SABR: Practical Comparison}
\label{tab:heston_vs_sabr}
\begin{tabular}{lcc}
\hline
\textbf{Property} & \textbf{Heston} & \textbf{SABR} \\\\
\hline
Pricing method & Monte Carlo/FFT & Closed-form approx \\\\
Typical pricing time & 50-200 ms & 0.1-1 ms \\\\
Mean reversion & Yes ($\kappa$) & No (GBM vol) \\\\
Accuracy & Exact (MC limit) & Approx (errors up to 100\%) \\\\
Term structure & Rich dynamics & Limited \\\\
Primary markets & Equities, FX & Rates, FX \\\\
\hline
\end{tabular}
\end{table}

\noindent \textbf{Choosing between models:}
\begin{itemize}
    \item \textbf{Heston}: When mean reversion matters (long-dated options), exact pricing required, or modeling equity derivatives
    \item \textbf{SABR}: When speed is critical (real-time trading), interest rate products, or market convention dictates
\end{itemize}
...
\begin{itemize}
    \item \textbf{Deep learning denoising}: Neural networks to filter MC noise, potentially reducing required paths by 50-70\%
    \item \textbf{Quasi-Monte Carlo}: Sobol sequences for variance reduction
    \item \textbf{Andersen QE scheme}: Exact variance simulation respecting Feller condition
    \item \textbf{GPU acceleration}: Parallel path generation for 100$\times$ speedups
\end{itemize}
